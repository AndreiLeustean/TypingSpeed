...
let typedText = "";
let currentWord = "";
let isCurrentWordCorrect = false;
...
function checkWordsMatch(typedText, targetWord) {
    return typedText === targetWord;
}

function updateColorBasedOnMatch(typedText, targetWord) {
    if (checkWordsMatch(typedText, targetWord)) {
...
function updateCurrentWord() {
    currentWord += generatedText[currentWordIndex];
}
...
function resetStateAfterWordCheck(wordLength) {
    generatedText = generatedText.slice(wordLength);
    document.getElementById("userInput").value = "";
    document.getElementById("textOfWords").innerHTML = generatedText;
    typedText = "";
    currentWord = "";
    currentWordIndex = 0;
}
...
function validateWord() {
    const wordLength = getWordLength(generatedText) + 1;
    if (getWordLength(generatedText) === getWordLength(typedText)
